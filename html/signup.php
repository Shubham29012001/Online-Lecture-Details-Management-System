...
if (isset($_REQUEST['Submit'])) 
{
    ...
    $query = "INSERT INTO user(id,Name,Username,Email,Password,token,status) VALUES ('','$name','$user','$email',MD5('$pass'), '$token', 'inactive')";

    $dupemail = "SELECT * FROM user WHERE Email = '$email';";
    $result = mysqli_query($conn, $dupemail);

    $emailrow = mysqli_num_rows($result);
